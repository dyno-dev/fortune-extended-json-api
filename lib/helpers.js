...
function attachQueries (request) {
  ...
  const relationshipDelimiter = options.relationshipDelimiter
  const allowRelationshipFilters = options.allowRelationshipFilters
  ...
  for (const parameter of Object.keys(query))
    ...
    // Attach match option.
      ...
      if (isRelationFilter(field, relationshipDelimiter)) {
        const relationPath = field
        if (!allowRelationshipFilters && filterType !== 'fuzzy-match')
...
function isRelationFilter ( field, relationshipDelimiter ) {
  return field.split(relationshipDelimiter).length > 1
}

function getRelationFilterSegments ( field, relationshipDelimiter ) {
  return field.split(relationshipDelimiter)
}
